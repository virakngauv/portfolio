...
The receiver uses one App-owned branch/PR for the batch and refuses unexpected paths or human-authored commits. It rebuilds the release tree from current main plus validated gitlinks and advances the branch without force-pushing. When newer upstream heads are ineligible, it retains pending candidate pins after rechecking their exact upstream CI and forward ancestry. Both retained and new pins are rebuilt on current main and included in the PR description, so strict up-to-date checks can pass. Pins already included or superseded on main are not carried backward. Missing auto-merge requests are retried for the validated head. Changes refresh CI before auto-merge. Manual edits belong on separate branches.
