...
This App contains a static portfolio component plus one service for the gateway and two game servers. The portfolio component installs the exact development toolchain from `pnpm-lock.yaml`, runs `pnpm build`, and publishes `dist/`; it has no runtime process or persistent data. Additional static projects can be separate components, each sourced directly from its own repo. The games' Next.js frontends remain separately hosted; this repository does not make them static exports. No DNS, accounts or deployments are created by the repository or its CI.
...
3. Add only the domains you actually own, and apply the DNS targets supplied by the App Platform domain setup. The example routes the portfolio at `virakngauv.com` and the backends at `game.pic-match.virakngauv.com` and `game.secrethitman.com`; game frontend domains remain separate. Configure exact Host ingress rules and matching backend `*_HOSTS` values. The gateway does not route unknown domains to a default game.
